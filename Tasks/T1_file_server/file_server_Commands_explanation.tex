...
\section{Client/Worker Configuration}

\noindent\textit{Note:} The following section details the complete workflow for each client in the cluster, from building the Docker image to executing the client application. This section is repeated for each client to demonstrate the independent configuration process on each worker node.

\subsection{Client 1 (First Worker Node)}
...
\noindent\textit{Explanation:} Creates a Docker image named \texttt{task1\_client} on the first worker node. The Dockerfile copies all files from the current directory (including client.py and test files) and sets environment variables for server connection configuration.

\subsubsection{Join Docker Swarm}
...
\noindent\textit{Explanation:} Joins the first worker node to the Docker Swarm cluster. The token (obtained from the manager node) authenticates the node, while \texttt{192.168.12.26:2377} specifies the manager node's address and Docker Swarm management port.

\subsubsection{Launch Client Container}

\vspace{0.3em}
\noindent\textbf{Run Client Container:}
\begin{lstlisting}
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
    \item \texttt{-it}: Runs container in interactive mode with terminal access
    \item \texttt{--rm}: Automatically removes the container upon termination
    \item \texttt{--name client1}: Assigns the container name "client1"
    \item \texttt{--hostname client1}: Sets hostname for network identification
    \item \texttt{--network 6231-net}: Connects container to the overlay network
    \item \texttt{bash}: Opens bash shell for manual command execution
\end{itemize}
...
\subsubsection{Execute Client Application}

\vspace{0.3em}
\noindent\textbf{Run Client Program:}
\begin{lstlisting}
python client.py
\end{lstlisting}

\noindent\textit{Explanation:} Executes the client application, which connects to the server using hostname resolution via Docker's DNS, creates a \texttt{client1} directory on the server, and transfers files to that directory.

\subsection{Client 2 (Second Worker Node)}
...
\noindent\textit{Explanation:} Creates a Docker image named \texttt{task1\_client} on the second worker node. Each worker node builds its own image independently to avoid dependency on image distribution mechanisms.
...
\noindent\textit{Explanation:} Joins the second worker node to the Docker Swarm cluster using the same token and manager node address as Client 1.
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
    \item \texttt{-it}: Runs container in interactive mode with terminal access
    \item \texttt{--rm}: Automatically removes the container upon termination
    \item \texttt{--name client2}: Assigns the container name "client2"
    \item \texttt{--hostname client2}: Sets unique hostname for network identification
    \item \texttt{--network 6231-net}: Connects container to the overlay network
    \item \texttt{bash}: Opens bash shell for manual command execution
\end{itemize}
...
\noindent\textit{Explanation:} Executes the client application, which connects to the server, creates a \texttt{client2} directory on the server, and transfers files to that directory.

\subsection{Client 3 (Third Worker Node)}
...
\noindent\textit{Explanation:} Creates a Docker image named \texttt{task1\_client} on the third worker node, completing the image build process for all worker nodes in the cluster.
...
\noindent\textit{Explanation:} Joins the third worker node to the Docker Swarm cluster using the same token and manager node address.
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
    \item \texttt{-it}: Runs container in interactive mode with terminal access
    \item \texttt{--rm}: Automatically removes the container upon termination
    \item \texttt{--name client3}: Assigns the container name "client3"
    \item \texttt{--hostname client3}: Sets unique hostname for network identification
    \item \texttt{--network 6231-net}: Connects container to the overlay network
    \item \texttt{bash}: Opens bash shell for manual command execution
\end{itemize}
...
\noindent\textit{Explanation:} Executes the client application, which connects to the server, creates a \texttt{client3} directory on the server, transfers files, and executes commands such as \texttt{wordcount about.txt}.
...
\begin{tabular}{lllll}
\toprule
\textbf{Container} & \textbf{Hostname} & \textbf{IP Address} & \textbf{Port} & \textbf{Host IP Address} \\
\midrule
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{./image.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{./client1.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{./client2.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{./client3.png}}
