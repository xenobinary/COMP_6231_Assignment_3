...
\subsection{Export Image for Distribution}

\vspace{0.3em}
\noindent\textbf{Save Image to Tar File:}
\begin{lstlisting}
docker save -o mpi-node.tar mpi-node
\end{lstlisting}

\noindent\textit{Explanation:} Since the MPI image is built on the manager node, it must be distributed to all worker machines in the Docker Swarm cluster. The \texttt{docker save} command exports the image to a tar archive file. The \texttt{-o} flag specifies the output filename, and \texttt{mpi-node} is the image name to export. This tar file will be transferred to each worker machine for loading.
...
\noindent\textit{Note:} The following section details the complete workflow for each worker physical machine in the cluster, from loading the Docker image to launching worker containers. This section is repeated for each physical worker machine to demonstrate the independent configuration process.

\subsection{Second Physical Computer - Workers 3, 4, 5}

\subsubsection{Load Docker Image}

\vspace{0.3em}
\noindent\textbf{Transfer Image File (Execute on Master Node):}
\begin{lstlisting}
# On master node (192.168.12.26)
scp mpi-node.tar user@192.168.12.16:~/
\end{lstlisting}

\noindent\textit{Explanation:} \textbf{This command is executed on the master node (192.168.12.26).} Transfer the \texttt{mpi-node.tar} file from the manager node to the second physical computer (192.168.12.16) using secure copy (scp). Replace \texttt{user} with the actual username on the worker machine.

\vspace{0.5em}
\noindent\textbf{Load Image from Tar (Execute on Worker Node):}
\begin{lstlisting}
# On worker node (192.168.12.16)
docker load -i mpi-node.tar
\end{lstlisting}

\noindent\textit{Explanation:} \textbf{This command is executed on the second physical computer (192.168.12.16).} Load the Docker image from the tar archive. The \texttt{-i} flag specifies the input file. This makes the \texttt{mpi-node} image available for container deployment on this worker machine.

\subsubsection{Join Docker Swarm (if not already joined)}

\vspace{0.3em}
\noindent\textbf{Join Worker to Swarm:}
\begin{lstlisting}
docker swarm join --token <worker-token> 192.168.12.26:2377
\end{lstlisting}

\noindent\textit{Explanation:} If this physical machine has not yet joined the Docker Swarm, use this command to join as a worker node. The \texttt{<worker-token>} was obtained from the manager node (via \texttt{docker swarm join-token worker}), and \texttt{192.168.12.26} is the IP address of the manager node.

\subsubsection{Prepare Required Files}

Place the following files in the working directory on this physical machine:
\begin{itemize}[leftmargin=*,itemsep=4pt]
    \item \textbf{Dataset:} \texttt{Books\_rating.csv}
    \item \textbf{MPI Scripts:} \texttt{q1\_t3.py, q2\_t3.py, q3\_t3.py, q4\_t3.py}
\end{itemize}

\subsubsection{Launch Worker Containers}
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
    \item \texttt{-d}: Runs container in detached mode (background)
    \item \texttt{--name mpi-worker3}: Names the container for easy reference
    \item \texttt{--hostname mpi-worker3}: Sets hostname for network identification
    \item \texttt{--network 6231-net}: Connects to the overlay network
    \item \texttt{-v "\$(pwd):/app:ro"}: Mounts current directory to /app as read-only, sharing dataset and scripts
    \item \texttt{mpi-node}: Uses the loaded image
\end{itemize}

\noindent\textit{Container IP:} \texttt{10.0.1.72}
...
\noindent\textit{Container IP:} \texttt{10.0.1.74}
...
\noindent\textit{Container IP:} \texttt{10.0.1.75}

\vspace{0.5em}
\noindent\textit{Explanation:} Three worker containers are launched on the second physical machine. Each container connects to the overlay network and has access to the dataset and scripts via the volume mount.
...
\begin{lstlisting}
# On master node (192.168.12.26)
scp mpi-node.tar user@192.168.12.5:~/
\end{lstlisting}

\noindent\textit{Explanation:} \textbf{This command is executed on the master node (192.168.12.26).} Transfer the \texttt{mpi-node.tar} file from the manager node to the third physical computer (192.168.12.5).

\vspace{0.5em}
\noindent\textbf{Load Image from Tar (Execute on Worker Node):}
\begin{lstlisting}
# On worker node (192.168.12.5)
docker load -i mpi-node.tar
\end{lstlisting}

\noindent\textit{Explanation:} \textbf{This command is executed on the third physical computer (192.168.12.5).} Load the Docker image from the tar archive.
...
\noindent\textit{Explanation:} Join this physical machine (192.168.12.5) to the Docker Swarm if not already joined.
...
\subsubsection{Launch Worker Container}
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
    \item \texttt{-d}: Runs container in detached mode (background)
    \item \texttt{--name mpi-worker6}: Names the container for easy reference
    \item \texttt{--hostname mpi-worker6}: Sets hostname for network identification
    \item \texttt{--network 6231-net}: Connects to the overlay network
    \item \texttt{-v "\$(pwd):/app:ro"}: Mounts current directory to /app as read-only
    \item \texttt{mpi-node}: Uses the loaded image
\end{itemize}

\noindent\textit{Container IP:} \texttt{10.0.1.80}

\vspace{0.5em}
\noindent\textit{Explanation:} Single worker container launched on the third physical machine, demonstrating the flexibility of distributing containers across different physical hosts.
...
\begin{lstlisting}
# On master node (192.168.12.26)
scp mpi-node.tar user@192.168.12.3:~/
\end{lstlisting}

\noindent\textit{Explanation:} \textbf{This command is executed on the master node (192.168.12.26).} Transfer the \texttt{mpi-node.tar} file from the manager node to the fourth physical computer (192.168.12.3).

\vspace{0.5em}
\noindent\textbf{Load Image from Tar (Execute on Worker Node):}
\begin{lstlisting}
# On worker node (192.168.12.3)
docker load -i mpi-node.tar
\end{lstlisting}

\noindent\textit{Explanation:} \textbf{This command is executed on the fourth physical computer (192.168.12.3).} Load the Docker image from the tar archive.
...
\noindent\textit{Explanation:} Join this physical machine (192.168.12.3) to the Docker Swarm if not already joined.
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
    \item \texttt{-d}: Runs container in detached mode (background)
    \item \texttt{--name mpi-worker7}: Names the container for easy reference
    \item \texttt{--hostname mpi-worker7}: Sets hostname for network identification
    \item \texttt{--network 6231-net}: Connects to the overlay network
    \item \texttt{-v "\$(pwd):/app:ro"}: Mounts current directory to /app as read-only
    \item \texttt{mpi-node}: Uses the loaded image
\end{itemize}

\noindent\textit{Container IP:} \texttt{10.0.1.85}
...
\noindent\textit{Container IP:} \texttt{10.0.1.84}
...
\noindent\textit{Container IP:} \texttt{10.0.1.65}
...
\noindent\textit{Container IP:} \texttt{10.0.1.67}

\vspace{0.5em}
\noindent\textit{Explanation:} Four worker containers are launched on the fourth physical machine, completing the 10-worker setup for scalability testing.
...
\begin{tabular}{lll}
\toprule
\textbf{Container Name} & \textbf{IP Address} & \textbf{Host IP Address}\\
\midrule
mpi-head & 10.0.1.76 & 192.168.12.26 \\
mpi-worker1 & 10.0.1.78 & 192.168.12.26 \\
mpi-worker2 & 10.0.1.79 & 192.168.12.26 \\
mpi-worker3 & 10.0.1.72 & 192.168.12.16 \\
mpi-worker4 & 10.0.1.74 & 192.168.12.16 \\
mpi-worker5 & 10.0.1.75 & 192.168.12.16 \\
mpi-worker6 & 10.0.1.80 & 192.168.12.5 \\
mpi-worker7 & 10.0.1.85 & 192.168.12.3 \\
mpi-worker8 & 10.0.1.84 & 192.168.12.3 \\
mpi-worker9 & 10.0.1.65 & 192.168.12.3 \\
mpi-worker10 & 10.0.1.67 & 192.168.12.3 \\
\bottomrule
\end{tabular}
